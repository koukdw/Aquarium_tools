...
| 0x1D         | Marker           | Value              | Mark `start` and `end` points of a `statement`, this is only used in the `main script` (those are probably autogenerated savepoints, the number of savepoints as well as current savepoint is saved in Save\\Script.dat) |
